...
After calling `.add_stage(...)`, all subsequent criterion methods (`.linear()`, `.direct()`, `.min_ratio()`, `.threshold()`, `.geometric_mean()`, `.inverse()`, `.custom()`) add criteria to **that** stage. Calling `.add_stage(...)` again starts a new stage.
